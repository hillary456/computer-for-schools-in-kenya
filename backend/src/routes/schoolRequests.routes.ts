...
import { AuthRequest, SchoolRequest, RequestStatus } from '../types.js';
...
router.post('/', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    const newRequest: Partial<SchoolRequest> = {
      user_id,
...
router.get('/all', requireAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const resultRequests = requests.map(r => ({
            ...r,
            user_name: r.users ? (r.users as { name: string }).name : null,
            users: undefined,
        }));
...
router.put('/', requireAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const valid_statuses: RequestStatus[] = ['pending', 'approved', 'fulfilled', 'rejected'];
